...
`plot_solution` - рисовать ли результат или нет. может принимать значения True или False. В первом случае график 
...
Что мы миеем: сейчас мы ищем решение относительно объемной концентрации, но удобнее всего 
нам работать со счетной концентрцией. Так понятнее и нагляднее представляется концентрация частиц. 
Для расчета объемной концентрации частиц необходимо знать как саму концентрацию, так и медианный радиус и 
полуширину распределения.
$$
mu = ln(rm)
V = (4/3*pi)N exp(3*mu+4.5*sigma**2)
muv = mu+3*sigma**2
V = (4/3*pi)N exp(3(muv-3sigma**2)+4.5sigma**2) = (4/3*pi)N exp(3muv-4.5sigma**2)
$$
